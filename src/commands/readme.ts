...
`
  ...
  static flags = {
    ...
    spaceSep: flags.boolean({ description: 'output space separated commands on docs' }),
  }
  ...
  async run() {
    ...
    this.spaceSeparated = flags.spaceSep
    if (this.spaceSeparated) {
      prepareSpaceSeparated()
    }
...
${config.name}/${process.env.OCLIF_NEXT_VERSION || config.version} ${process.platform}-${process.arch} node-v${
...
\`\`\`\n`,
  ...
  private commandUsage(config: Config.IConfig, command: Config.Command): string {
    ...
    const defaultUsage = () => {
      ...
      // .filter(([, v]) => !v.hidden)
      return compact([
        this.spaceSeparated ? command.id.replace(':', ' ') : command.id,
        command.args
          .filter(a => !a.hidden)
          .map(a => arg(a))
          .join(' '),
